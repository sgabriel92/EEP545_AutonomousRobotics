...
class ParticleFilter():
  ...
  def __init__(self, n_particles, n_viz_particles,
    ...
    self.pose_sub  = rospy.Subscriber("/car/initialpose", PoseWithCovarianceStamped, self.clicked_pose_cb, queue_size=1)
  ...
  def expected_pose(self):
    ...
    for i in range(len(self.particles)):
      ...
      theta_x += np.sin(self.particles[i][2])
      theta_y += np.cos(self.particles[i][2])
  ...
  def clicked_pose_cb(self, msg):
    ...
    for i in range(len(self.particles)):
      ...
      sample_y = 0
      while not in_bounds:
        sample_x = int(np.random.normal(map_pose[:, 0], .005))
        sample_y = int(np.random.normal(map_pose[:, 1], .005))
        in_bounds = self.permissible_region[sample_y][sample_x]
      
      self.particles[i, 0] = sample_x
      self.particles[i, 1] = sample_y
      self.particles[i, 2] = map_pose[0, 2]

      # #self.particles[i] = [sample_x, sample_y, map_pose[:, 2]]
      # for j, value in enumerate(map_pose[:, 2]):
      #   self.particles[i, j] = [sample_x, sample_y, value]
