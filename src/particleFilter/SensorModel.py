...
# Z_SHORT = 0.125  # Weight for short reading
# Z_MAX = 0.15  # Weight for max reading
# Z_RAND = 0.025  # Weight for random reading
# SIGMA_HIT = 0.7  # Noise value for hit reading
# Z_HIT = 4.0  # Weight for hit reading
...
class SensorModel:
  ...
  def lidar_cb(self, msg):
    ...
    laser_ranges = np.where(np.isnan(laser_ranges), self.MAX_RANGE_METERS, laser_ranges)
    laser_ranges = np.where(laser_ranges == 0.0, self.MAX_RANGE_METERS, laser_ranges)

    downsample_ranges = np.zeros(len(laser_ranges) // ds, dtype=np.float32)
    downsample_ranges = laser_ranges[0: len(laser_ranges)-ds: ds]

    downsample_angles = np.zeros(len(laser_ranges) // ds, dtype=np.float32)
    downsample_angles = np.arange(msg.angle_min, msg.angle_max, ds * msg.angle_increment)

    # obs = (downsample_ranges, downsample_angles)
    obs = np.ascontiguousarray(downsample_ranges, dtype=np.float32), np.ascontiguousarray(downsample_angles, dtype=np.float32)
  ...
  def precompute_sensor_model(self, max_range_px):
    ...
    print('precompute_sensor_model', sensor_model_table.shape)
    ...
    z_kt = np.zeros(sensor_model_table.shape, dtype=np.float32)
    z_kt_star = np.zeros(sensor_model_table.shape, dtype=np.float32)

    z_kt_star[:] = np.arange(0.0, table_width)
    z_kt[:] = z_kt_star[:].T  # pk_THUNDER
    zmax = max_range_px + 1
    z_array = np.array([Z_HIT, Z_SHORT, Z_MAX, Z_RAND])

    for d in range(table_width):
      for r in range(table_width):
        # calc prob: p = [z_hit, z_short, z_max, z_rand] dot [p_hit, p_short, p_max, p_rand]
        (ztk, ztk_star) = (d, r)
        p_hit, p_short, p_max, p_rand = 0.0, 0.0, 0.0, 0.0
        if 0 <= ztk < zmax:
          p_hit = (1.0 / (2.0 * np.pi * (SIGMA_HIT ** 2.0)) ** 0.5) * (np.exp(-0.5 * (((ztk - ztk_star)**2.0) / (SIGMA_HIT**2.0))))
        ...
        sensor_model_table[r, d] = np.dot(
          z_array,
          np.array([p_hit, p_short, p_max, p_rand])
        )
