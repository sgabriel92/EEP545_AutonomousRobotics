...
class KinematicMotionModel:
  ...
  def motion_cb(self, msg):
    ...
    speed_noise = np.random.normal(curr_speed, KM_V_NOISE, len(self.particles))
    delta_noise = np.random.normal(curr_delta, KM_DELTA_NOISE, len(self.particles))

    x_noise = np.random.normal(0.0, KM_X_FIX_NOISE, len(self.particles))
    y_noise = np.random.normal(0.0, KM_Y_FIX_NOISE, len(self.particles))
    theta_noise = np.random.normal(0.0, KM_THETA_FIX_NOISE, len(self.particles))

    beta = np.arctan(0.5 * np.tan(delta_noise))
    dt = (msg.header.stamp - self.last_vesc_stamp).to_sec()

    km_theta = speed_noise / self.CAR_LENGTH * np.sin(2 * beta) * dt
    ...
    self.particles[:, 0] = self.particles[:, 0] + km_x + x_noise
    self.particles[:, 1] = self.particles[:, 1] + km_y + y_noise
    self.particles[:, 2] = self.particles[:, 2] + km_theta + theta_noise
...
TEST_DT = 1  # seconds
# TEST_STEERING_ANGLE = -0.27 # radians
# TEST_DT = 2.0 # seconds
