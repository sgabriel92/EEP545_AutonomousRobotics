...
class PlannerNode(object):
  ...
  def add_orientation(self, plan):
    ...
    oriented_plan = np.zeros((plan.shape[0],3),dtype=np.float32)
    ...
    if plan.shape[0] >= 2:  
      ...
      plan_diffs = np.zeros(plan.shape, np.float32)
      plan_diffs[0:plan_diffs.shape[0]-1] = plan[1:plan.shape[0]]-plan[0:plan.shape[0]-1]
      plan_diffs[plan_diffs.shape[0]-1] = np.array([np.cos(self.target_yaw), np.sin(self.target_yaw)], dtype=np.float32) 
    
      avg_diffs = np.empty(plan_diffs.shape, dtype=np.float32)
